...
contract MoodNftTest is Test {
    ...
    function testFlipMoodWorksFine() public {
        vm.prank(USER);
        moodNft.mintNft();
        vm.prank(USER);
        moodNft.flipMood(0);
        console.log(
            "svg generated from the mood NFt contract.",
            moodNft.tokenURI(0)
        );
        console.log(
            "svg that is already present in the current contract",
            SAD_SVG_URI
        );
